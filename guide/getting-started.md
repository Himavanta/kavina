...
const views = ref([]);
const widgets = Object.values(example);
</script>
...
<template>
  <Display v-model:views="views" v-model:model="model" :widgets="widgets" mode="display" />
</template>
```
...
```vue
<Design v-model:views="views" v-model:dark="isDark" v-model:theme="themeColor" :widgets="widgets" />
```
